...
/*
 * Cambia la cantidad de tickets (prioridad) de un proceso
 * Si el proceso esta corriendo solo puedo reducirle la prioridad
 * Si el proceso esta RUNNABLE puedo aumentarle o reducirle la prioridad
 * En ningun caso puede tener menos de 1 ticket o mas de 50. Si se pasa, se
 * ajusta a estos limites. tickets puede ser negativo o positivo, si es negativo
 * se reduce la prioridad, si es positivo se aumenta
 */
static int
sys_modify_tickets(int pid, int tickets)
{
	for (int i = 0; i < NENV; i++) {
		if (envs[i].env_id == pid) {
			if (envs[i].env_status == ENV_RUNNING &&
			    tickets < 0) {  // es el proceso actual y solo puedo reducirle la prioridad
				envs[i].tickets += tickets;
				if (envs[i].tickets <
				    MIN_TICKETS)  // no puede tener menos de 1 ticket
				...
				if (envs[i].tickets <
					...
					envs[i].tickets = MIN_TICKETS;
				if (envs[i].tickets >
				    MAX_TICKETS)  // no puede tener mas de 50 tickets
					envs[i].tickets = MAX_TICKETS;
				break;
			}
		}
	}
...
static int
sys_get_process_ticket(int pid)
{
	for (int i = 0; i < NENV; i++) {
		if (envs[i].env_id == pid) {
			return envs[i].tickets;
		}
	}
	return -1;
}
...
static int
sys_reduce_priority(int tickets)
{
	if (tickets < 0 || tickets > curenv->tickets) {
		return -1;
	}
	curenv->tickets = tickets;
	return 0;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_yield:
		sys_yield();  // No return
	case SYS_modify_tickets:
		sys_modify_tickets(a1, a2);
	case SYS_get_process_ticket:
		return sys_get_process_ticket(a1);
